<x-app-layout>
    <title>Biometric Digital ID - First Bank Account details</title>
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="mb-0 fw-bold text-primary">First Bank Account Details</h4>
                            <p class="text-muted mb-0">View and manage First Account request</p>
                        </div>
                        <div class="d-flex gap-2">
                            <a href="{{ route('first-account.index') }}" class="btn btn-light">
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="card shadow-sm border-0 mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-4">
                            {{-- Passport Image (Maintaining Passport Style) --}}
                            @if($enrollmentInfo->passport_url)
                            <div class="col-12 text-center mb-3">
                                <label class="form-label text-muted small text-uppercase fw-bold d-block">Passport Photograph</label>
                                <img src="{{ $enrollmentInfo->passport_url }}" class="rounded shadow-sm border p-1" style="max-width: 200px; max-height: 200px; object-fit: cover;" onerror="this.src='{{ asset('assets/img/profiles/avatar-01.jpg') }}'">
                                <div class="mt-2">
                                    <a href="{{ $enrollmentInfo->passport_url }}" target="_blank" class="btn btn-sm btn-outline-primary">View Full Size</a>
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="col-md-6">
                                ...
                                <p class="fw-medium mb-0">{{ $enrollmentInfo->number ?? $enrollmentInfo->phone_number ?? 'N/A' }}</p>
                            </div>
                            ...
                            {{-- Residential Address --}}
                            <div class="col-12">
                                <label class="form-label text-muted small text-uppercase fw-bold">Residential Address</label>
                                <div class="p-3 bg-light rounded border">
                                    @if($enrollmentInfo->street_name)
                                        {{ $enrollmentInfo->house_number ?? '' }} {{ $enrollmentInfo->street_name }}, {{ $enrollmentInfo->city ?? '' }}
                                    @else
                                        {{ $enrollmentInfo->description ?? 'N/A' }}
                                    @endif
                                </div>
                            </div>
        ...
        <div class="modal fade" id="updateStatusModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg">
                    ...
                    <form method="POST" action="{{ route('first-account.update', $enrollmentInfo->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
