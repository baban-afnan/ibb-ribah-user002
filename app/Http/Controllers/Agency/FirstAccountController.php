...
{
    /**
     * List first_account requests with filters and pagination
     */
    public function index(Request $request)
    {
        ...
        $statusCounts = [
            'pending'    => AgentService::where('service_type', 'first_account')->where('status', 'pending')->count(),
            'processing' => AgentService::where('service_type', 'first_account')->where('status', 'processing')->count(),
            'resolved'   => AgentService::where('service_type', 'first_account')->whereIn('status', ['resolved', 'successful'])->count(),
            'rejected'   => AgentService::where('service_type', 'first_account')->whereIn('status', ['rejected', 'failed'])->count(),
        ];

        return view('first-account.index', compact('enrollments', 'search', 'statusFilter', 'statusCounts'));
    }

    /**
     * Show details of a single first_account request
     */
    public function show($id)
    {
        ...
        return view('first-account.view', compact('enrollmentInfo', 'statusHistory', 'user'));
    }

    /**
     * Update the status of a first_account request
     */
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                $filePath = $file->storeAs('first_account-files', $fileName, 'public');
                $baseUrl = rtrim(config('app.url'), '/');
                $fileUrl = $baseUrl . '/storage/first_account-files/' . $fileName;
            }
            ...
            DB::commit();
            return redirect()->route('first-account.index')
    ...
    /**
     * Export pending first_account requests as CSV
     */
    public function exportCsv()
    {
        $fileName = 'first_account-pending-requests-' . date('Y-m-d') . '.csv';
        $enrollments = AgentService::where('service_type', 'first_account')
        ...
        $callback = function() use($enrollments) {
            ...
            fputcsv($file, ['Reference', 'BVN', 'NIN', 'Phone', 'LGA', 'State', 'Submission Date']);
            ...
            foreach ($enrollments as $enrollment) {
                fputcsv($file, [
                    ...
                    $enrollment->bvn,
                    $enrollment->nin,
                    $enrollment->number ?? $enrollment->phone_number,
                    $enrollment->lga,
                    $enrollment->state,
                    $enrollment->submission_date
                ]);
            }
    ...
    /**
     * Export pending first_account requests as Excel
     */
    public function exportExcel()
    {
        ...
        $sheet->setCellValue('B1', 'BVN');
        $sheet->setCellValue('C1', 'NIN');
        $sheet->setCellValue('D1', 'Phone');
        $sheet->setCellValue('E1', 'LGA');
        ...
        $row = 2;
        foreach ($enrollments as $enrollment) {
            $sheet->setCellValue('A' . $row, $enrollment->reference);
            $sheet->setCellValue('B' . $row, $enrollment->bvn);
            $sheet->setCellValue('C' . $row, $enrollment->nin);
            $sheet->setCellValue('D' . $row, $enrollment->number ?? $enrollment->phone_number);
            $sheet->setCellValue('E' . $row, $enrollment->lga);
            $sheet->setCellValue('F' . $row, $enrollment->state);
            $sheet->setCellValue('G' . $row, $enrollment->submission_date);
            $row++;
        }
        ...
        $fileName = 'first_account-pending-requests-' . date('Y-m-d') . '.xlsx';
